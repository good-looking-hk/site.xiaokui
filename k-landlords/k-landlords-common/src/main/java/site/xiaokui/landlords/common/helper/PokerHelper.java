package site.xiaokui.landlords.common.helper;
...
import site.xiaokui.landlords.common.util.ListUtil;
    ...
    public static List<PokerSell> parsePokerSells(List<Poker> pokers) {
        ...
        //all single or double
        {
            ...
            for (int index = 0; index < pokers.size(); index++) {
                ...
                if (count == 1) {
                    pokerSells.add(new PokerSell(SellType.SINGLE, ListUtil.getList(sellPokers), poker.getLevel().getLevel()));
                } else if (count == 2) {
                    pokerSells.add(new PokerSell(SellType.DOUBLE, ListUtil.getList(sellPokers), poker.getLevel().getLevel()));
                } else if (count == 3) {
                    pokerSells.add(new PokerSell(SellType.THREE, ListUtil.getList(sellPokers), poker.getLevel().getLevel()));
                } else if (count == 4) {
                    pokerSells.add(new PokerSell(SellType.BOMB, ListUtil.getList(sellPokers), poker.getLevel().getLevel()));
                }
                lastLevel = level;
            }
        }
        ...
        //king boom
        {
            if (size > 1) {
                if (pokers.get(size - 1).getLevel() == PokerLevel.LEVEL_BIG_KING && pokers.get(size - 2).getLevel() == PokerLevel.LEVEL_SMALL_KING) {
                    pokerSells.add(new PokerSell(SellType.KING_BOMB, ListUtil.getList(new Poker[]{pokers.get(size - 2), pokers.get(size - 1)}), PokerLevel.LEVEL_BIG_KING.getLevel()));
                }
            }
        }
                        ...
                        if (increase_1 >= minLenght) {
                            for (int s = 0; s <= increase_1 - minLenght; s++) {
                                ...
                                for (int subIndex = 0; subIndex <= increase_1 - len; subIndex++) {
                                    List<Poker> pokers = ListUtil.getList(sellPokers_1.subList(subIndex * width, (subIndex + len) * width));
