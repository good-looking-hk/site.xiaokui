...
import site.xiaokui.blog.entity.wrapper.SysUserWrapper;
...
public class UserController extends AbstractController {
    ...
    @RequestMapping(ConstantController.LIST)
    @ResponseBody
    ...
    @RequiresPermissions(USER_PREFIX + ConstantController.ADD)
    ...
    @RequiresPermissions(USER_PREFIX + ConstantController.REMOVE)
    ...
    public ResultEntity edit(Long id, String name, String email, String sex, String password,
        ...
        user.setEmail(email);
        user.setSex(code);
        user.setSalt(BaseController.SHIRO.fastSalt());
        user.setPassword(BaseController.SHIRO.md5(password, user.getSalt()));
        user.setRoleId(roleId);
    ...
    @RequiresPermissions(USER_PREFIX + ConstantController.RESET_PASSWORD)
    @PostMapping(ConstantController.RESET_PASSWORD)
    @ResponseBody
    ...
    @PostMapping(ConstantController.UPDATE)
    @ResponseBody
    public ResultEntity update(String username, String sex, String blogSpace, String password, String newPassword, String selfDescription) {
        ...
        if (this.isNotEmpty(password) && this.isNotEmpty(newPassword)) {
            ...
            String oldPassword = BaseController.SHIRO.md5(password, temp1.getSalt());
            if (oldPassword.equals(temp1.getPassword())) {
                user.setSalt(BaseController.SHIRO.fastSalt());
                user.setPassword(BaseController.SHIRO.md5(newPassword, user.getSalt()));
            } else {
